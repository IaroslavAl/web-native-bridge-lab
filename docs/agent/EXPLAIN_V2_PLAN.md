...
## Initial-packet isolation and planning evidence (before run87 review)
...
Tasks: write failing semantic/data/history tests first; extract technical controls with all their old assertions; implement D1–D4 plus bootstrap fallback and D6 web layout. Test D1 bootstrap selection in both compiled builds (single exact diagnostics mode versus absent/unknown/duplicate mode), fresh production client lifecycle, and diagnostics clearing only the demo continuity key without running/consuming demo history. Keep summaries compatible with technical consumers. Synchronously guard duplicate activation. Show no-new only after real reload comparison. Fresh B and repeat do not invent A. Tests use explicit native doubles and storage/navigation ports; no production fake boundary or server timer. README documents the D5 long-press/VoiceOver entry, fixed destination, reload/retry and return, explicitly pending N rather than suggesting a browser URL is installed bridge access.
...
- `README.md` installed engineering entry/return and native recovery wording; `docs/design/explain-v2/native-evidence.md` (new). No concurrent work on SameBinaryTests or README with W/R. `scripts/verify` and its tests remain unchanged in N: the D5 footer action uses the already-served production root/query, with no new host synchronization. Stop for bounded plan correction if implementation discovers a runner prerequisite, rather than silently depending on R.

Implement D5: compact light shell, actual native version/build, accessible generic load/retry, revoked-page interaction suppressed, static asset failure never called success. Add the closed shell-local WebSurface and version-footer context menu/custom actions with exact destinations/identifiers from D5; native `lab.reload` and failed-load retry preserve that selection. Shared test flow taps web update CTA for the primary A→B proof. For Stage5 only, after B/finish, long-press `lab.nativeVersion`, select `lab.openDiagnostics`, verify real Diagnostics, reload and verify mode preserved, run every existing production diagnostic assertion, then select `lab.openDemo` and verify current B without diagnostics before signalling `matrix-observed`. Stage2/3 still reload the demo root after the host swaps in their raw harness; do not redirect those modes to TechnicalPanel. No hardcoded catalog/quote handling in native. Do not rename protocol codes to localize shell text.
...
Stage5 includes actual A/B plus installed menu entry, diagnostics-preserving reload, retained HTTP/body/category/timeout/fast-and-slow-cancel outcomes and return to B. Existing host checks after `matrix-observed` still verify installed-file equality and real diagnostic backend events; no later R runner prerequisite. W retains the hostile-body literal-text/no-elements assertion in TechnicalPanel tests; Stage3 retains its real hostile echo probe. Do not misreport either as a new Stage5 hostile-response injection. N's focused shell/WebKit tests cover default/selected fixed URLs, failure retry and return even before diagnostics boots; installed UI evidence covers the real footer/menu and accessibility names/actions. Stage4 is required once for this mature native slice because load presentation touches lifecycle-adjacent code; host Swift package tests alone do NOT compile/prove the SwiftUI/WebKit adapter. Use dedicated owned Simulators and verify teardown. Do not alter production inspectability, adapter trust or timeout policy for testing. Stop/replan if callbacks disturb lifecycle fences, actual WK reload doesn't preserve intended session semantics, or failed-page UI remains interactive. Request same-card review.
...
| EX-08 | D1 bootstrap mode/history contract and all TechnicalPanel assertions including inert hostile body | N: installed footer entry → diagnostics reload → retained Stage5 body/category/slow-only-cancel → return to B; focused route failure/retry tests; retained Stage3 hostile echo and Stage4 trust/lifecycle/privacy with exact regression rationale |

## R1 review correction (docs/spec only)

Rejected planning head `d3f4b243049983d293112024f127f5a99ec2cfeb` is preserved as an ancestor. Independent review run87 (iosverifier, card comment166) correctly found that a README query URL was unreachable from the fixed-root installed shell, leaving N's Stage5 gate dependent on an unsettled interface. That head passed OpenSpec/schema checks but was NOT spec-approved. Coordinator confirmed the finding; this is the first same-root rework, not new owner scope or design approval.

Correction: D1/W owns bootstrap mode/history behavior; D5/N owns one concrete footer menu/custom-action entry, closed URLs, return and selection-preserving retry, plus the Stage5 UI helper. Existing runner control flow is sufficient: production B remains served, `finish` precedes diagnostics, and `matrix-observed` follows entry/outcomes/return; no `scripts/verify` change or R dependency is needed. EX-06/08 and tasks now match. Shell/policy/runner and source assertions were re-read; runtime proof is still assigned work, not a claim made by this correction.

Rework isolation: clean initial rejected head and same absolute worktree/branch/common-dir reverified. Actual lead PID78422 argv is `deliverylead -m gpt-6-astra --provider openai-codex --reasoning high`, matching configuration. Prior reviewer PID76257 Astra/high was observed in run87 evidence; the new reviewer is not launched yet. Environment lookup again exposes task ID but not separate workspace/DB/board values. Exact correction head and newly executed validation results are recorded on the re-review transition; historical checks below refer to the initial packet, not runtime acceptance or approval of R1.

## Executed initial-packet planning checks (rejected head above)
